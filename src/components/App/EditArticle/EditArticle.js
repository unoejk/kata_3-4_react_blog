...
const EditArticle = () => {
	...
	const onSubmit = async (data) => {
		...
		if (isEditMode) {
			await updateArticle(slug, token, article)
				.then(() => {
					// console.log(res)
					reset()
			...
			await addArticle(token, article)
				.then(() => {
					// console.log(res)
					reset()
					setTagsList(initialTasList.slice(0))
		...
		return (
			<form className={style.editForm} onSubmit={handleSubmit(onSubmit)}>
				...
				<label className={style.editForm__textLabel}>
					...
					<input
						className={classNames(style.editForm__textInput, {
				...
				<label className={style.editForm__textLabel}>
					...
					<textarea
						className={classNames(style.editForm__textInput, style.editForm__textArea, {
				...
				<div className={style.editForm__controlPanel}>
					...
					{tagsList.map((val, num) => (
						<div className={classNames(style.editForm__controlRow)} key={val.key}>
							<label>
								<input
									className={classNames(
							...
							<button
								className={classNames(style.editForm__controlBtn, style['editForm__controlBtn--red'], {
							...
							<button
								className={classNames(style.editForm__controlBtn, style['editForm__controlBtn--blue'])}
								onClick={(e) => addTag(e)}
							>
								Add tag
							</button>
				...
				<div className={style.editForm__submitRow}>
					<button className={style.editForm__submitBtn} type={'submit'}>
						Create
					</button>
					<Spin className={classNames(style.editForm__spin, { disabled: !isSecondLoading })} size="large" />
					<p className={classNames(style.editForm__errorMessage, { disabled: !isSecondError })}>
						{isSecondError}
					</p>
				</div>
			</form>
